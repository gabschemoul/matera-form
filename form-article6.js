...
function prefillForm() {
  ...
  const lastStepValidation2 = (counter) => {
    ...
    const setHeigthActionsButtons2 = () => {
      if (submitButton2 && counter === LAST_STEP ) {
        submitButton2.style.top = `${STEPS_NAVIGATION_BUTTON_TOP_NEXT[`step_${counter}`]}em`
				submitButton2.style.display = "block"
      }
			if (submitButton2 && counter !== LAST_STEP) submitButton2.style.display = "none"
      if (nextButton2) nextButton2.style.top = `${STEPS_NAVIGATION_BUTTON_TOP_NEXT[`step_${counter}`]}em`
      if (previousButton2) previousButton2.style.top = `${STEPS_NAVIGATION_BUTTON_TOP_PREV[`step_${counter}`]}em`
    }
    ...
    if (previousButton) previousButton.addEventListener("click", decrementCounter)
	  if (previousButton2) previousButton2.addEventListener("click", decrementCounter2)
  });
